...
table_region_move_start_end <- function(data, column, start = 3, end = 1){
  ...
  for(i in 1:12686){
    ...
    # we only care about origin == 3 
    m <- 1
    if(!is.na(origin)){ # make sure origin is not NA
      while(isTRUE(origin != start)){ # we only care about origin == 3
        origin <- as.numeric(temp[m, column])
    ...
    for(j in n:25){
      if(!is.na(as.numeric(temp[j,column]))){
        if(isTRUE(as.numeric(temp[j,column]) == end)){ # only matches with  == 1
